...
- ` ```mermaid ` blocks are drawn as diagrams once the block is complete, in the current light or dark theme. Each diagram can switch to its source, copy it, download SVG or PNG, and open enlarged with zoom (buttons or Ctrl+wheel) and drag to pan. A diagram that fails to parse, is over 50,000 characters, or takes more than 10 seconds shows its source and the error instead. Diagrams are drawn by Mermaid's compact build in an isolated sandbox page and shown as images, so diagram text can't run scripts or style the panel. The compact build leaves out mindmap and architecture diagrams and math (KaTeX); those show as source.
